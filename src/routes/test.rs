...
use crate::subsystems::drivetrain_actions::{CONFIG, TileToMMExt, forward, turn_to_point};
...
async fn route(robot: &mut crate::Robot) -> () {
    ...
    robot.drivetrain.action(forward(0.5.tiles(), CONFIG)).await;
    // robot.drivetrain.action(forward(0.1, CONFIG)).await;
    robot
        .drivetrain
        .action(turn_to_point(Point2::new(0.0, 1.0), CONFIG))
        .await;
    robot.drivetrain.action(forward(0.1.tiles(), CONFIG)).await;
}
